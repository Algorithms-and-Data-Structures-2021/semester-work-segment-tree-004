...
static const string kBenchmarkResultOutputFile = "segment_tree_sum_getSum_average_benchmark.csv";
...
int main() {
  ...
  while (getline(benchmark_settings_file, line)){
    ...
    for (int i = 0; i < number_of_passes; ++i){
      int random_left = distr(gen) % count_of_samples;
      const auto time_point_before = chrono::steady_clock::now();

      // здесь находится участок кода, время которого необходимо замерить
      segmentTreeSum->getSum( random_left,count_of_samples-1);
